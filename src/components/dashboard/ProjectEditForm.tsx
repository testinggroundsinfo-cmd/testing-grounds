...
export default function ProjectEditForm({ project }: Props) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!user || user.id !== project.owner_id) throw new Error(t("projectEdit.noPermission"));
      ...
      if (coverFile instanceof File && coverFile.size > 0) {
        if (!["image/jpeg", "image/png", "image/webp"].includes(coverFile.type) || coverFile.size > 5 * 1024 * 1024) {
          throw new Error(t("projectEdit.coverRequirements"));
        }
      ...
      if (!response.ok) {
        throw new Error(responseBody.error || t("projectEdit.unableToUpdate"));
      }
      setMessage(t("projectEdit.updated"));
      router.refresh();
      router.push("/dashboard/projects");
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : t("projectEdit.unableToUpdate"));
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-2xl border border-white/10 bg-ink-800 p-6">
      <label className="block text-sm">{t("projectEdit.title")}<input name="title" required defaultValue={project.title} className="mt-1 w-full rounded-lg border border-white/10 bg-ink-900 px-3 py-2" /></label>
      <label className="block text-sm">{t("projectEdit.slug")}<input name="slug" required defaultValue={project.slug} className="mt-1 w-full rounded-lg border border-white/10 bg-ink-900 px-3 py-2" /></label>
      <label className="block text-sm">{t("projectEdit.shortDescription")}<input name="short_description" defaultValue={project.short_description ?? ""} maxLength={180} className="mt-1 w-full rounded-lg border border-white/10 bg-ink-900 px-3 py-2" /></label>
      ...
      <fieldset className="space-y-3 rounded-xl border border-white/10 bg-ink-900/50 p-4">
        <legend className="text-sm font-medium">{t("projectEdit.mirrorsLegend")}</legend>
        <p className="text-xs text-zinc-400">{t("projectEdit.mirrorsHint")}</p>
        {alternativeLinks.map((link, index) => (
          <div key={index} className="grid gap-2 sm:grid-cols-[1fr_1.5fr_auto]">
            ...
            <input
              ...
              className="rounded-lg border border-white/10 bg-ink-900 px-3 py-2 text-sm"
            />
            <button type="button" onClick={() => setAlternativeLinks((current) => current.filter((_, itemIndex) => itemIndex !== index))} className="rounded-lg border border-red-400/30 px-3 py-2 text-sm text-red-300">{t("common.remove")}</button>
          </div>
        ))}
        <button type="button" onClick={() => setAlternativeLinks((current) => [...current, { label: "", url: "" }])} className="rounded-lg border border-white/10 px-3 py-2 text-sm">{t("common.addMirror")}</button>
      </fieldset>
      <label className="block text-sm">{t("projectEdit.newCover")}<input name="cover_file" type="file" accept="image/jpeg,image/png,image/webp" className="mt-1 block w-full rounded-lg border border-white/10 bg-ink-900 px-3 py-2 text-sm" /></label>
      {project.cover_url ? <img src={project.cover_url} alt="" className="aspect-video w-full rounded-lg object-cover" /> : null}
      <label className="flex items-center gap-2 text-sm"><input name="is_published" type="checkbox" defaultChecked={project.is_published} /> {t("projectEdit.published")}</label>
      <label className="flex items-center gap-2 text-sm">
        <input name="notify_new_bugs" type="checkbox" defaultChecked={project.notify_new_bugs ?? true} />
        {t("projectEdit.notifyBugs")}
      </label>
      <button disabled={saving} className="rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-ink-950 disabled:opacity-50">{saving ? t("common.saving") : t("projectEdit.saveChanges")}</button>
